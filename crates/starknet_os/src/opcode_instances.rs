...
pub fn get_opcode_instances(runner: &CairoRunner) -> OpcodeInstanceCounts {
    let Ok(trace) = runner.get_relocatable_trace() else {
        eprintln!("Failed to get relocatable trace. Returning zero count.");
        return OpcodeInstanceCounts { blake_opcode_count: 0 };
    };

    let memory = runner.get_relocatable_memory();
    ...
    let count = trace
        ...
        .filter(|entry| {
            (|| {
                ...
                let value = memory.get(seg)?.get(entry.pc.offset)?.as_ref()?;
